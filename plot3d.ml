...
type cameraView = { phi : float; theta : float; zoomFactor : float;  cameraposition : point3D; };;
...
type colorChoice = HSV of hsv_color | RGB of rgb_color;;
...
let plotTriangle mesh ind ((x,y,z) : point3D) cameraParameters (lightDirection : point3D) style chosenColor=
...
let plotMesh mesh options =
    ...
    and chosenColor = match options.colorchoice with
    ...
    and dSort = distanceSort mesh options.cameraview.cameraposition in
        for i=0 to mesh.nTria - 1 do
            let ind = snd (dSort.(i)) in
                plotTriangle mesh ind options.cameraview.cameraposition cameraParameters options.lightdirection options.style chosenColor;
