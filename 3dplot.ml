...
(*** mesh concatenation ***)
let concat mesh1 mesh2 =
    let nVert = mesh1.nVert + mesh2.nVert
    and nTria = mesh1.nTria + mesh2.nTria in
        let positions = Array.make_matrix nVert 3 0.0
        and triangles = Array.make_matrix nTria 3 0 in
            for i=0 to nVert-1 do
                if(i<mesh1.nVert) then begin
                    positions.(i).(0) <- mesh1.positions.(i).(0);
                    positions.(i).(1) <- mesh1.positions.(i).(1);
                    positions.(i).(2) <- mesh1.positions.(i).(2);
                end else begin
                    positions.(i).(0) <- mesh2.positions.(i-mesh1.nVert).(0);
                    positions.(i).(1) <- mesh2.positions.(i-mesh1.nVert).(1);
                    positions.(i).(2) <- mesh2.positions.(i-mesh1.nVert).(2);
                end
            done;
            for i=0 to nTria-1 do
                if(i<mesh1.nTria) then begin
                    triangles.(i).(0) <- mesh1.triangles.(i).(0);
                    triangles.(i).(1) <- mesh1.triangles.(i).(1);
                    triangles.(i).(2) <- mesh1.triangles.(i).(2);
                end else begin
                    triangles.(i).(0) <- mesh1.nVert + mesh2.triangles.(i-mesh1.nTria).(0);
                    triangles.(i).(1) <- mesh1.nVert + mesh2.triangles.(i-mesh1.nTria).(1);
                    triangles.(i).(2) <- mesh1.nVert + mesh2.triangles.(i-mesh1.nTria).(2);
                end
            done;
            {nVert = nVert; nTria = nTria; positions = positions; triangles = triangles; } ;;